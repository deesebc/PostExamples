...
public class WebServiceTest {
	...
	public void validateBody() {
		...
		given().when().get("/JerseyExample/rest/book/list").then().body("library.findAll { it.id < 2 }.author",
				hasItems("Isaac Asimov"));
	}

	@Test
	public void validateRoot() {
		get("/JerseyExample/rest/book").then().root("[0]").body("author", equalTo("Isaac Asimov")).and().body("id",
				equalTo(1));
	}
