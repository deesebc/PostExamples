...
public class BookClient {
	...
	public List<Book> getAll() throws ClientProtocolException, IOException {
		...
		java.lang.reflect.Type listType = new com.google.gson.reflect.TypeToken<ArrayList<Book>>() {}.getType();
	...
	public int saveOne(final Book book) throws ClientProtocolException, IOException {
		...
		return Request.Post(URL).bodyString(bookJson, ContentType.APPLICATION_JSON).useExpectContinue()
				.version(HttpVersion.HTTP_1_1).execute().returnResponse().getStatusLine().getStatusCode();
	}
	...
	public int updateOne(final Integer id, final Book book) throws ClientProtocolException, IOException {
		...
		return Request.Put(URL + "/" + id).body(requestEntity).addHeader("Conten-Type", "application/json").execute()
