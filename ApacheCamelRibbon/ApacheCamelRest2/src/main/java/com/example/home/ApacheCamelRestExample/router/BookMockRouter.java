...
public class BookMockRouter extends RouteBuilder {
	...
	rest().get("book").produces(MediaType.APPLICATION_JSON_VALUE).route().bean(DBMockRepository.class, "getAll(})")
		.marshal().json();
	...
	rest().post("book").type(Book.class).produces(MediaType.APPLICATION_JSON_VALUE).route().routeId("postBookRoute")
		.bean(DBMockRepository.class, "create(${body})").log("---creating a book---");

	rest().delete("book/{id}").produces(MediaType.APPLICATION_JSON_VALUE).route().bean(DBMockRepository.class,
		"remove(${header.id})");

	rest().put("book/{id}").type(Book.class).produces(MediaType.APPLICATION_JSON_VALUE).route().choice().when()
		.simple("${header.id} < 1").bean(BookMockRouter.class, "negativeId").otherwise()
